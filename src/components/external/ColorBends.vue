...
const applyProps = () => {
  ...
  const toVec3 = (hex: string) => {
    ...
    const char = (index: number) => h.charAt(index)
    const v: [number, number, number] =
      h.length === 3
        ? [
            parseInt(char(0).repeat(2), 16),
            parseInt(char(1).repeat(2), 16),
            parseInt(char(2).repeat(2), 16),
          ]
  ...
  const colors = (material.uniforms.uColors?.value as THREE.Vector3[] | undefined) ?? []
  for (let i = 0; i < MAX_COLORS; i++) {
    const vec = colors[i]
    if (!vec) continue
    const color = arr[i]
    if (color) vec.copy(color)
    else vec.set(0, 0, 0)
  }
